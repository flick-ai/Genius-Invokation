...
class GoldenTroupeEntity(Artifact):
    ...
    def on_end(self, game: 'GeniusGame'):
        if game.active_player_index == self.from_player.index:
            if not self.from_character.is_active:
                self.recompense = min(self.max_usage, self.recompense + 2)
...
class GoldenTroupe(ArtifactCard):
    id: int = 312026
    name: str = "Golden Troupe"
    name_ch = "黄金剧团"
    time = 4.7
    ...
    def __init__(self) -> None:
        super().__init__()
        self.artifact_entity = GoldenTroupeEntity
